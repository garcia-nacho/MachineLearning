...
faces<- Input %>%
  layer_conv_2d(filters=1, kernel_size=c(10,10), activation='sigmoid', padding='same',strides=c(1,1),data_format='channels_last')%>% 
  layer_conv_2d(filters=filters*2, kernel_size=c(10,10), activation='sigmoid', padding='same',strides=c(2,2),data_format='channels_last')%>%
...
Output<- z %>%
  ...
  layer_dense(units = prod(75,75,filters*8), activation = "sigmoid") %>%
...
vae_loss <- function(x, x_decoded_mean_squash) {
  ...
  xent_loss <- 2.0 * dimensions[1]* dimensions[1] *
...
Samp1.gen <- predict(vae, array(df[15,,,], dim = c(1,300,300,1)), batch_size = 10)
Samp1.gen<-as.matrix(Samp1.gen)
Samp1.gen<-matrix(Samp1.gen, ncol = 300)
...
plot(intermediate_output$V2,intermediate_output$V3)
